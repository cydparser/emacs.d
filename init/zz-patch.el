...
(defmacro init-patch (package broken-version &rest body)
  ...
  `(if (version-list-<= ,broken-version (pkg-info-package-version ,package))
       (with-eval-after-load (symbol-name ,package) ,@body)
     (message "Package %s may no longer be broken" package)))
